...
import { ProfileSiteProps } from '@/controllers/types/controller.type'; // 1. Impor tipe data dari sumber yang benar
                    ...
                    <article className="mb-12" key={itemBeritaUtama.id}>
                        <div className="relative lg:h-[25rem] h-[11rem] overflow-hidden rounded-2xl px-16 pt-16 pb-8 mb-8 shadow-md lg:px-24">
                            <picture className="pointer-events-none absolute inset-0 -z-10 dark:hidden">
                                <img src={itemBeritaUtama.lampiran ? `https://cms.depok.go.id/upload/${itemBeritaUtama.lampiran}` : '/img/kecamatan/dsw/kesehatan.png'} alt={itemBeritaUtama.title} className="h-full w-full object-cover" />
                            </picture>
                        ...
                        <article>
                            <div className="lg:flex lg:space-x-8">
                                <div className="article-content lg:w-2/3">
                                    <h2 className="text-3xl font-bold font-display text-jacarta-700 dark:text-white">{itemBeritaUtama.title}</h2>
                                    <div className="mt-4 mb-8 text-jacarta-700 dark:text-gray-300 text-justify" dangerouslySetInnerHTML={{ __html: itemBeritaUtama?.content || '' }}>
                                    </div>
                                    <div className="mb-16 flex items-center">
                                        ...
                                        <div className="flex space-x-2">
                                            ...
                                            <a href={cleanProfileSite?.Facebook ?? '#'} target="_blank" rel="noopener noreferrer" className="group inline-flex h-10 w-10 items-center justify-center rounded-full border border-jacarta-100 bg-white transition-colors hover:border-transparent hover:bg-accent dark:border-jacarta-600 dark:bg-jacarta-700 dark:hover:bg-accent">
                                                <svg aria-hidden="true" focusable="false" className="h-5 w-5 fill-jacarta-300 group-hover:fill-white" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                                            ...
                                            <a href={cleanProfileSite?.Instagram ?? '#'} target="_blank" rel="noopener noreferrer" className="group inline-flex h-10 w-10 items-center justify-center rounded-full border border-jacarta-100 bg-white transition-colors hover:border-transparent hover:bg-accent dark:border-jacarta-600 dark:bg-jacarta-700 dark:hover:bg-accent">
                                                <svg aria-hidden="true" focusable="false" className="h-5 w-5 fill-jacarta-300 group-hover:fill-white" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
                                ...
                                <div className="mt-12 lg:w-1/3">
                                    {pengumumanPopuler && pengumumanPopuler.slice(0, 3).map((item: any) => {
                                        return (
                                            <div className="mb-4" key={item.id}>
                                                <div className="mb-4 flex rounded-2.5xl border border-jacarta-100 bg-white p-4 dark:border-jacarta-600 dark:bg-jacarta-700">
                                                    <img
                                                        src={item.lampiran ? `https://cms.depok.go.id/upload/${item.lampiran}` : '/img/kecamatan/dsw/kesehatan.png'}
                                                        alt={item.title}
