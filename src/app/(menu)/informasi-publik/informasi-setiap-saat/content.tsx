...
export default function Content({ SetiapSaat }: { SetiapSaat: InformasiSetiapSaatProps[] | null }) {
  ...
  const filteredSetiapSaat = SetiapSaat ? SetiapSaat.filter((item: any) => {
  ...
  return (
    ...
    {filteredSetiapSaat && filteredSetiapSaat.filter((sort: any) => sort.ParentId === null).reverse().map((item: any, index : number) => {
        ...
        return (
        <div role="table"
            className="w-full overflow-y-auto my-5 rounded-lg rounded-tl-none border border-jacarta-100 bg-white text-sm dark:border-jacarta-600 dark:bg-jacarta-700 dark:text-white" key={index}>
            ...
            {filteredSetiapSaat && filteredSetiapSaat.filter((sort: any) => sort.ParentId === item.content_id).reverse().map((parent : any, count : number) => {
                return(
                <>
                    <div className="flex" role="row" key={count}>
                    <div
                    ...
                    <div
                        ...
                        role="cell">
                        <a href={`https://cms.depok.go.id/upload/file/${parent.uploaddokumen}`} download>
                        <button
                    ...
                    {SetiapSaat && SetiapSaat.filter((indx: any) => indx.ParentId === parent.content_id).map((child : any, bet : number) => {
                        ...
                        <div className="ml-12!" style={{ marginLeft: "50px" }} key={bet}>
                            ...
                            <div
                                ...
                                role="cell">
                                <a href={`https://cms.depok.go.id/upload/file/${child.uploaddokumen}`} download>
                                <button
                                className="group mr-2.5 my-2.5 inline-flex items-center rounded-xl bg-white px-4 py-3 hover:border-transparent hover:bg-blue hover:text-white dark:border-jacarta-600 dark:bg-jacarta-700 dark:text-white dark:hover:border-transparent dark:hover:bg-blue">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"
                                    className="mr-2 h-4 w-4 fill-jacarta-700 group-hover:fill-white dark:fill-white">
