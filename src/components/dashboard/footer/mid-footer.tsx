...
export default async function MidFooter() {
    ...
    // Buat URL Google Maps dengan aman
    const googleMapsUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(
        cleanProfileSite?.Alamat ?? "Kota Depok"
    )}`;
    ...
    return (
        <div className="col-span-full sm:col-span-3 md:col-span-3 md:col-start-6">
            ...
            <ul className="flex flex-col space-y-1 dark:text-jacarta-300">
                <li>
                    <Link href={googleMapsUrl} className="flex" target="_blank" rel="noopener noreferrer">
                        <svg
                        ...
                        <span> {cleanProfileSite?.Alamat ?? "Gedung Dibaleka II, Jl. Margonda No.54, Depok"}</span>
                    </Link>
                </li>
                <li className="pt-3">
                    <a
                        href={`tel:${cleanProfileSite?.Telp?.replace(/\D/g, "") ?? "+622129402276"}`}
                        className="flex"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                        ...
                        &nbsp;
                        {cleanProfileSite?.Telp ?? "(021) 29402276"}
                    </a>
                </li>
                <li className="pt-3">
                    <a
                        href={`mailto:${cleanProfileSite?.Email ?? "diskominfo@depok.go.id"}`}
                        className="flex"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                        ...
                        &nbsp;
                        <span>{cleanProfileSite?.Email ?? "diskominfo@depok.go.id"}</span>
                    </a>
                </li>
            </ul>
        </div>
